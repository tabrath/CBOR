    ...
    public static class DataUtilities
    {
        ...
        public static string GetUtf8String(byte[] bytes, int offset, int bytesCount, bool replace)
        {
            ...
            if (offset > bytes.Length)
                throw new ArgumentException("offset (" + offset + ") is more than " + bytes.Length);

            if (bytesCount < 0)
                throw new ArgumentException("bytesCount (" + bytesCount + ") is less than 0");

            if (bytesCount > bytes.Length)
                throw new ArgumentException("bytesCount (" + bytesCount + ") is more than " + bytes.Length);

            if (bytes.Length - offset < bytesCount)
                throw new ArgumentException("bytes's length minus " + offset + " (" + (bytes.Length - offset) + ") is less than " + bytesCount);
        ...
        public static byte[] GetUtf8Bytes(string str, bool replace) => GetUtf8Bytes(str, replace, false);
        ...
        public static byte[] GetUtf8Bytes(string str, bool replace, bool lenientLineBreaks)
        {
            ...
            if (!lenientLineBreaks && str.Length == 1)
            {
                ...
                if (c <= 0x80)
                    ...
                    return new byte[] { (byte)(0xe0 | ((c >> 12) & 0x0f)), (byte)(0x80 | ((c >> 6) & 0x3f)), (byte)(0x80 | (c & 0x3f)) };
            }
            ...
            try
            {
                ...
                {
                    if (WriteUtf8(str, 0, str.Length, ms, replace, lenientLineBreaks) != 0)
                        throw new ArgumentException("Unpaired surrogate code point");
        ...
        public static int CodePointBefore(string str, int index) => CodePointBefore(str, index, 0);
        ...
        public static int CodePointBefore(string str, int index, int surrogateBehavior)
        {
            ...
            if ((c & 0xfc00) == 0xdc00 && index - 2 >= 0 && str[index - 2] >= 0xd800 && str[index - 2] <= 0xdbff)
        ...
        public static int CodePointAt(string str, int index) => CodePointAt(str, index, 0);
        ...
        public static int CodePointCompare(string strA, string strB)
        {
            ...
            for (var i = 0; i < len; ++i)
            {
                ...
                else
                {
                    ...
                    if ((ca & 0xfc00) == 0xd800 && i + 1 < strA.Length && strA[i + 1] >= 0xdc00 && strA[i + 1] <= 0xdfff)
                    {
                        ca = 0x10000 + ((ca - 0xd800) << 10) + (strA[i + 1] - 0xdc00);
                    }

                    if ((cb & 0xfc00) == 0xd800 && i + 1 < strB.Length && strB[i + 1] >= 0xdc00 && strB[i + 1] <= 0xdfff)
                    {
                        cb = 0x10000 + ((cb - 0xd800) << 10) + (strB[i + 1] - 0xdc00);
                    }
        ...
        public static int WriteUtf8(string str, int offset, int length, Stream stream, bool replace) => WriteUtf8(str, offset, length, stream, replace, false);
        ...
        public static int WriteUtf8(string str, int offset, int length, Stream stream, bool replace, bool lenientLineBreaks)
        {
            ...
            if (offset > str.Length)
                throw new ArgumentException("offset (" + offset + ") is more than " + str.Length);

            if (length < 0)
                throw new ArgumentException("length (" + length + ") is less than " + "0");

            if (length > str.Length)
                throw new ArgumentException("length (" + length + ") is more than " + str.Length);

            if (str.Length - offset < length)
                throw new ArgumentException("str.Length minus offset (" + (str.Length - offset) + ") is less than " + length);
            ...
            for (int index = offset; index < endIndex; ++index)
            {
                ...
                {
                    if ((c & 0xfc00) == 0xd800 && index + 1 < endIndex && str[index + 1] >= 0xdc00 && str[index + 1] <= 0xdfff)
                    {
                        // Get the Unicode code point for the surrogate pair
                        c = 0x10000 + ((c - 0xd800) << 10) + (str[index + 1] - 0xdc00);
                        ++index;
        ...
        public static int ReadUtf8FromBytes(byte[] data, int offset, int bytesCount, StringBuilder builder, bool replace)
        {
            if (data == null)
            ...
            if (bytesCount > data.Length)
                throw new ArgumentException("bytesCount (" + bytesCount + ") is more than " + data.Length);

            if (data.Length - offset < bytesCount)
                throw new ArgumentException("data.Length minus offset (" + (data.Length - offset) + ") is less than " + bytesCount);
        ...
        public static string ReadUtf8ToString(Stream stream) => ReadUtf8ToString(stream, -1, true);

        /// <include file='../docs.xml'
        /// path='docs/doc[@name="M:PeterO.DataUtilities.ReadUtf8ToString(System.IO.Stream,System.Int32,System.Boolean)"]/*'/>
        public static string ReadUtf8ToString(Stream stream, int bytesCount, bool replace)
        {
            var builder = new StringBuilder();
            if (DataUtilities.ReadUtf8(stream, bytesCount, builder, replace) == -1)
                throw new IOException("Unpaired surrogate code point found.", new ArgumentException("Unpaired surrogate code point found."));
        ...
        public static int ReadUtf8(Stream stream, int bytesCount, StringBuilder builder, bool replace)
        {
            if (stream == null)
                throw new ArgumentNullException(nameof(stream));
