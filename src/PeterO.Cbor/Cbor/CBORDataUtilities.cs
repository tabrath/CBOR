...
namespace PeterO.Cbor {
  ...
  public static class CBORDataUtilities
  {
    ...
    public static CBORObject ParseJSONNumber(string str) => ParseJSONNumber(str, false, false);

    /// <include file='../../docs.xml'
    /// path='docs/doc[@name="M:PeterO.Cbor.CBORDataUtilities.ParseJSONNumber(System.String,System.Boolean,System.Boolean)"]/*'/>
    public static CBORObject ParseJSONNumber(string str, bool integersOnly, bool positiveOnly) => ParseJSONNumber(str, integersOnly, positiveOnly, false);
    ...
    public static CBORObject ParseJSONNumber(string str, bool integersOnly, bool positiveOnly, bool preserveNegativeZero)
    {
...
#if DEBUG
          if (mantInt < 0)
            throw new ArgumentException("mantInt (" + mantInt + ") is less than 0");
#endif
      ...
      {
        EInteger bigmant = (mant == null) ? ((EInteger)mantInt) : mant.AsBigInteger();
        EInteger bigexp = (newScale == null) ? ((EInteger)newScaleInt) : newScale.AsBigInteger();
        if (negative)
          bigmant = -(EInteger)bigmant;

        EDecimal edec;
        edec = EDecimal.Create(bigmant, bigexp);
